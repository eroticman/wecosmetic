...
function banner_add() {
...

	{
		...
		if(!empty($_FILES['covImgMB']))

		{

			$timestamp = time();
			$file_name		= $timestamp . $_FILES['covImgMB']["name"];
			...
			$moved = move_uploaded_file($_FILES['covImgMB']["tmp_name"], $file_path);
...

			{
				...
				$sql = "UPDATE db_banner SET img_cover_mb = '{$file_name}' WHERE id = '{$banner_id}'";
