...
if ( !empty($fileList) ) {
	...
	foreach ($fileList as $img) {
		...
		$sql = "INSERT INTO db_product_img (product_id, img_name, order_id, created) VALUE ('{$product_id}', '{$img['name']}', '{$i}', '{$updated}')";
...
foreach($FileUploader->getRemovedFiles('file') as $key=>$value) {
	$sqlDel = "DELETE FROM db_product_img WHERE product_id = '{$product_id}' AND img_name = '{$value['name']}'";
